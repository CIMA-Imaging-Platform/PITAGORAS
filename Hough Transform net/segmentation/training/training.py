...
from segmentation.training.ranger import Ranger
...
def get_loss(loss_function):
    ...
    criterion = {'hough_transform': border_criterion, 'cell': cell_criterion}
    ...
    for epoch in range(max_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for samples in dataloader[phase]:
                ...
                img_batch, hough_label_batch, cell_label_batch = samples
                img_batch = img_batch.to(device)
                cell_label_batch, hough_label_batch = cell_label_batch.to(device), hough_label_batch.to(device)
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    hough_transform_pred_batch, cell_pred_batch = net(img_batch)
                    loss_hough_transform = criterion['hough_transform'](hough_transform_pred_batch, hough_label_batch)
                    loss_cell = criterion['cell'](cell_pred_batch, cell_label_batch)
                    loss = loss_hough_transform + loss_cell
            ...
            if phase == 'train':
                ...
                if epoch_loss < best_loss:
                    ...
                    if configs['num_gpus'] > 1:
                        ...
                        torch.save(net.state_dict(), str(path_models / (str(configs['run_name']) + '.pth')))
